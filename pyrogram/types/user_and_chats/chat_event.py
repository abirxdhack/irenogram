...
class ChatEvent(Object):
    ...
    async def _parse(
        ...
        if isinstance(action, raw.types.ChannelAdminLogEventActionChangeAbout):
            ...
            created_forum_topic = types.ForumTopic._parse(client, action.topic, {}, users, chats)
            action = enums.ChatEventAction.CREATED_FORUM_TOPIC

        elif isinstance(action, raw.types.ChannelAdminLogEventActionEditTopic):
            old_forum_topic = types.ForumTopic._parse(client, action.prev_topic, {}, users, chats)
            new_forum_topic = types.ForumTopic._parse(client, action.new_topic, {}, users, chats)
