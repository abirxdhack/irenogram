...
class TCPFull(TCP):
    ...
    async def send(self, data: bytes, *args) -> None:
        frame  = struct.pack("<II", len(data) + 12, self.seq_no) + data
        frame += struct.pack("<I", crc32(frame) & 0xFFFFFFFF)
        self.seq_no += 1
    ...
    async def recv(self, length: int = 0) -> Optional[bytes]:
        async with self._recv_lock:
            ...
            total_length = struct.unpack("<I", hdr)[0]
            ...
            frame    = hdr + rest
            checksum = struct.unpack("<I", frame[-4:])[0]
            body     = frame[:-4]
