            ...
            for i in reply_markup.inline_keyboard:
                for j in i:
                    ...
                    if isinstance(j, types.InlineKeyboardButtonBuy):
                        has_buy_button = True
                    elif isinstance(j, types.InlineKeyboardButton) and getattr(j, "pay", False):
                        has_buy_button = True
            if not has_buy_button:
                text = "Pay"
                if currency == "XTR":
                    prices_list = prices if not isinstance(prices, types.LabeledPrice) else [prices]
                    prices_total = sum(price.amount for price in prices_list)
