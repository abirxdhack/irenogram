...
class Session:
    ...
    async def start(self):
        while True:
            ...
            try:
                ...
                self.recv_task = asyncio.get_running_loop().create_task(
                    self.recv_worker()
                )
                ...
                self.ping_task = asyncio.get_running_loop().create_task(
                    self.ping_worker()
                )
    ...
    async def recv_worker(self):
        ...
        while True:
            ...
            if packet is None or len(packet) == 4:
                ...
                if self.is_started.is_set():
                    reason = (
                        ...
                        if packet is None
                        else f"transport error {-Int.read(BytesIO(packet))}"
                    )
            ...
            asyncio.get_running_loop().create_task(self.handle_packet(packet))
        ...
        while True:
            ...
            if isinstance(result, raw.types.RpcError):
                ...
                if isinstance(
                    ...
                    inner = inner.query
                RPCError.raise_it(result, type(inner))
