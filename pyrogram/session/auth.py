...
class Auth:
    ...
    def pack(data: TLObject) -> bytes:
        ...
        return (
            bytes(8)
            + _STRUCT_Q.pack(MsgId())
            + _STRUCT_I.pack(len(body))
            + body
        )
        ...
        while True:
            ...
            try:
                ...
                tmp_aes_key = (
                    ...
                    + sha1(server_nonce_bytes + new_nonce_bytes).digest()[:12]
                )

                tmp_aes_iv = (
                    sha1(server_nonce_bytes + new_nonce_bytes).digest()[12:]
                    + sha1(new_nonce_bytes + new_nonce_bytes).digest()
                    + new_nonce_bytes[:4]
                )
                ...
                server_salt = aes.xor(new_nonce_bytes[:8], server_nonce[:8])
